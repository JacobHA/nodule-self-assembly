...
def hstack_neighbours(left_infos, right_infos, offset):
    ...
    for i in range(n):
        ...
        out_values[out_cur+left:out_cur +
                   full] = right_values[right_cur:right_cur+right]
        out_sizes[i] = full
        left_cur += left
        right_cur += right
        out_cur += full
...
def compute(poss, radii, neighbours_sizes, neighbours_values):
    ...
    particle_corsp_overlaps = np.empty(
        neighbours_values.size, dtype=np.float64)
...
def ends_gap(poss, radii):
    ...
    neighbours_sizes, neighbours_values = vstack_neighbours(
        hstack_neighbours(small_small_neighbours,
                          small_big_neighbours, split_ind),
    ...
    # on a `BallTree(poss)`.
    res = reorder_neighbours(
        neighbours_sizes, neighbours_values, index, reverse_index)
